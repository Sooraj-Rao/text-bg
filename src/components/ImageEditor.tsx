...
export default function ImageEditor({ imageUrl }: ImageEditorProps) {
  ...
  const drawCanvas = () => {
    ...
    // If we have a mask, apply it to the text and image
    if (maskPoints.length > 2) {
      ...
      // Re-draw image and text inside the mask
      ctx.drawImage(imageRef.current, 0, 0);
      ctx.fillText(text, position.x, position.y + fontSize);
      ctx.restore();
    }
  ...
  const handleDrawMask = () => {
    ...
    setMaskPoints([]); // Start fresh drawing points for mask
    setLastPoint(null); // Reset last point for smooth drawing
  };
